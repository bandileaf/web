...
Batch item: `{"word":"react","action":"keep"}` or `{"word":"react","action":"split","prefix":"re","prefix_meaning":"다시","root":"act","root_meaning":"행동하다","suffix":null}`. For a NEW root also give `"root_pos":["verb","noun"]` (stored in `morphemes.pos text[]`: any of noun/verb/adj/adv; free roots like `grant` get every real part of speech, bound roots like `dict` the one their meaning implies; prefixes/suffixes stay empty). Part texts are the **actual spelling** (`ac`, not `ad`); the script maps variants to the representative.
